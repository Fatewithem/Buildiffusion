        ...
        for i, batch in enumerate(progress_bar):
            ...
            with accelerator.accumulate(model):
                ...
                with torch.cuda.amp.autocast(enabled=False):  # Disable mixed precision
                    ...
                    loss = \
                        model(pointcloud, masks=masks, camera=camera, images=images)
...
def sample(
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        result, image, gt = model.forward_sample(pointcloud, masks, camera, images)

        image_to_save = image[0].permute(2, 0, 1).cpu()
        ...
        pcd_gt.points = o3d.utility.Vector3dVector(gt.points_packed().cpu().numpy())
        pcd_gt.colors = o3d.utility.Vector3dVector(gt.features_packed().cpu().numpy().astype(np.float32))
