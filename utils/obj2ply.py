...
def convert_obj_to_point_cloud(obj_file_path, ply_file_path, target_scale, num_points=150000):
...
# def convert_obj_to_point_cloud(obj_file_path, ply_file_path, target_scale, num_points=150000):
#     """
#     从OBJ文件中均匀采样点云点，并保存为PLY文件，保留颜色信息。
#     包含居中与缩放的处理步骤。
#
#     Parameters:
#         obj_file_path (str): 输入的OBJ文件路径。
#         ply_file_path (str): 输出的PLY文件路径。
#         target_scale (float): 缩放到的目标大小。
#         num_points (int): 采样点的数量。
#     """
#     # 读取OBJ文件，自动加载对应的MTL文件（如果存在）
#     mesh = trimesh.load(obj_file_path, force='mesh')
#     if not isinstance(mesh, trimesh.Trimesh):
#         print("Failed to load the OBJ file. Please check the file path and format.")
#         return
...
#         print("Texture applied successfully.")
#     else:
#         print("No texture or vertex colors found in the OBJ/MTL file.")
#
#     # 进行均匀采样
#     point_cloud = mesh_o3d.sample_points_poisson_disk(num_points)
#
#     # 保存点云为PLY文件，包含颜色
#     o3d.io.write_point_cloud(ply_file_path, point_cloud)
#     print(f"Successfully converted {obj_file_path} to point cloud and saved as {ply_file_path}")
...
if __name__ == "__main__":
    ...
    #
    #             # 调用转换函数
    #             convert_obj_to_point_cloud(obj_file_path, ply_file_path, 40.0)
