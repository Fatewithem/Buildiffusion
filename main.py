...
def main(cfg: ProjectConfig):
    ...
    if cfg.run.mode == 'vis' or cfg.run.vis_before_training:
        visualize(
            cfg=cfg,
            model=model,
...
def visualize(
    *,
    ...
    model.eval()
    metric_logger = training_utils.MetricLogger(delimiter="  ")
    progress_bar: Iterable[dict] = metric_logger.log_every(dataloader_vis, cfg.run.print_step_freq, "Vis")
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        R = euler_angles_to_matrix(rotation, "XYZ")
        ...
        for i in range(2):
            ...
            }, ncols=2, viewpoint_cameras=camera, pointcloud_max_points=100_000)
...
def sample(
    *,
    ...
    # Eval mode
    model.eval()
    progress_bar: Iterable[dict] = tqdm(dataloader, disable=(not accelerator.is_main_process))
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        for sample_idx in range(cfg.run.num_samples):
            ...
            for i in range(len(output)):  # len(output)
                ...
                # metadata = dict(index=i, sequence_name=sequence_name, sequence_category=sequence_category,
