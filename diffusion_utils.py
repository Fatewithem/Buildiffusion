...
# 计算输入数据 x 在指定的高斯分布上的离散化对数似然； 将输入的连续数据离散化，并计算它在指定的均值和标准差下的概率
...
@torch.no_grad()
def visualize_distance_transform(path_stem: str, images: torch.Tensor) -> str:
...
@torch.no_grad()
def visualize_image(
        path_stem: str, images: torch.Tensor,
        # Union[torch.Tensor, float] 的作用是表明函数参数 mean 和 std 可以是两种类型之一
...
def render_pointcloud_batch_pytorch3d(
    ...
    # Create a points renderer by compositing points using a weighted compositor (3D points are
