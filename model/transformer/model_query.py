...
class MultiModalQueryModel(PointCloudProjectionModel):
    ...
    def forward(
        ...
        planes = torch.stack(planes, dim=0) / 10.0

        # loss = F.mse_loss(output, plane_tensor)
        # loss, _ = chamfer_distance(output, plane_tensor)

        loss = compute_loss(output, planes, self.target_points_per_query)
    ...
    def forward_sample(
        ...
        output = self.output_layer(prediction, target_num_points_list)  # [B, 9000, 3]

        output = convert_to_tensor(output)

        result = self.tensor_to_point_cloud(output, denormalize=True, unscale=True)

        planes = torch.stack(planes, dim=0)

        plane = self.tensor_to_point_cloud(planes, denormalize=True, unscale=False)

        return result, output, plane
